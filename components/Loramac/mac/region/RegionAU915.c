...
static bool VerifyRfFreq(uint32_t freq)
{
    ...
    // Rx frequencies
    if ((freq < AU915_FIRST_RX1_CHANNEL) || (freq > AU915_LAST_RX1_CHANNEL)
        || (((freq - (uint32_t)AU915_FIRST_RX1_CHANNEL)
             % (uint32_t)AU915_STEPWIDTH_RX1_CHANNEL)
            != 0)) {
        return false;
    }
...
PhyParam_t RegionAU915GetPhyParam(GetPhyParams_t *getPhy)
{
    ...
    case PHY_NEXT_LOWER_TX_DR: {
        RegionCommonGetNextLowerTxDrParams_t nextLowerTxDrParams = {
            ...
            .MinDr =
                (int8_t)((getPhy->UplinkDwellTime == 0) ? AU915_TX_MIN_DATARATE : AU915_DWELL_LIMIT_DATARATE),
    ...
    case PHY_RETRANSMIT_TIMEOUT: {
        phyParam.Value =
            (REGION_COMMON_DEFAULT_RETRANSMIT_TIMEOUT
             + randr(
    ...
    case PHY_BEACON_CHANNEL_FREQ: {
        phyParam.Value = RegionBaseUSCalcDownlinkFrequency(
            getPhy->Channel, AU915_BEACON_CHANNEL_FREQ, AU915_BEACON_CHANNEL_STEPWIDTH);
        break;
    }
...
{
    Band_t bands[AU915_MAX_NB_BANDS] = {AU915_BAND0};
    ...
    case INIT_TYPE_DEFAULTS: {
        ...
        memcpy1(
            (uint8_t *)RegionBands,
            (uint8_t *)bands,
            sizeof(Band_t) * AU915_MAX_NB_BANDS);
        ...
        for (uint8_t i = AU915_MAX_NB_CHANNELS - 8; i < AU915_MAX_NB_CHANNELS; i++) {
            ...
            RegionNvmGroup2->Channels[i].Frequency =
                915900000 + (i - (AU915_MAX_NB_CHANNELS - 8)) * 1600000;
        ...
        RegionCommonChanMaskCopy(
            RegionNvmGroup2->ChannelsMask,
            RegionNvmGroup2->ChannelsDefaultMask,
            CHANNELS_MASK_SIZE);

        // Copy into channels mask remaining
        RegionCommonChanMaskCopy(
            RegionNvmGroup1->ChannelsMaskRemaining,
            RegionNvmGroup2->ChannelsMask,
            CHANNELS_MASK_SIZE);
        break;
    }
    ...
    case INIT_TYPE_ACTIVATE_DEFAULT_CHANNELS: {
        ...
        for (uint8_t i = 0; i < CHANNELS_MASK_SIZE; i++) {   // Copy-And the channels mask
...
bool RegionAU915Verify(VerifyParams_t *verify, PhyAttribute_t phyAttribute)
{
    ...
    case PHY_TX_POWER: {
        ...
        return RegionCommonValueInRange(
            verify->TxPower, AU915_MAX_TX_POWER, AU915_MIN_TX_POWER);
    }
    case PHY_DUTY_CYCLE: {
...
void RegionAU915ApplyCFList(ApplyCFListParams_t *applyCFList)
{
    ...
    for (uint8_t chMaskItr = 0, cntPayload = 0; chMaskItr <= 4;
         chMaskItr++, cntPayload += 2) {
        RegionNvmGroup2->ChannelsMask[chMaskItr] =
            (uint16_t)(0x00FF & applyCFList->Payload[cntPayload]);
        RegionNvmGroup2->ChannelsMask[chMaskItr] |=
            (uint16_t)(applyCFList->Payload[cntPayload + 1] << 8);
        if (chMaskItr == 4) {
            RegionNvmGroup2->ChannelsMask[chMaskItr] =
                RegionNvmGroup2->ChannelsMask[chMaskItr] & CHANNELS_MASK_500KHZ_MASK;
        }
        // Set the channel mask to the remaining
        RegionNvmGroup1->ChannelsMaskRemaining[chMaskItr] &=
            RegionNvmGroup2->ChannelsMask[chMaskItr];
    }
}
...
bool RegionAU915ChanMaskSet(ChanMaskSetParams_t *chanMaskSet)
{
    ...
    case CHANNELS_MASK: {
        RegionCommonChanMaskCopy(
            RegionNvmGroup2->ChannelsMask,
            chanMaskSet->ChannelsMaskIn,
            CHANNELS_MASK_SIZE);

        RegionNvmGroup2->ChannelsDefaultMask[4] =
            RegionNvmGroup2->ChannelsDefaultMask[4] & CHANNELS_MASK_500KHZ_MASK;
        RegionNvmGroup2->ChannelsDefaultMask[5] = 0x0000;
...
void RegionAU915ComputeRxWindowParameters(
    int8_t            datarate,
    ...
    RxConfigParams_t *rxConfigParams)
{
    ...
    rxConfigParams->Bandwidth =
        RegionCommonGetBandwidth(rxConfigParams->Datarate, BandwidthsAU915);
    ...
    uint8_t            *nbBytesParsed)
{
    ...
    if ((linkAdrParams.Datarate < DR_6)
        && (RegionCommonCountChannels(channelsMask, 0, 4) < 2)) {
    ...
    status = RegionCommonLinkAdrReqVerifyParams(
        &linkAdrVerifyParams,
        &linkAdrParams.Datarate,
        &linkAdrParams.TxPower,
        &linkAdrParams.NbRep);
...
uint8_t RegionAU915RxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq)
{
    ...
    if (RegionCommonValueInRange(
            rxParamSetupReq->Datarate, AU915_RX_MIN_DATARATE, AU915_RX_MAX_DATARATE)
        == false) {
    ...
    if (RegionCommonValueInRange(
            rxParamSetupReq->DrOffset, AU915_MIN_RX1_DR_OFFSET, AU915_MAX_RX1_DR_OFFSET)
        == false) {
...
LoRaMacStatus_t RegionAU915NextChannel(
    ...
    TimerTime_t      *aggregatedTimeOff)
{
    ...
    if (RegionCommonCountChannels(RegionNvmGroup1->ChannelsMaskRemaining, 0, 4)
        == 0) {   // Reactivate default channels
        RegionCommonChanMaskCopy(
            RegionNvmGroup1->ChannelsMaskRemaining, RegionNvmGroup2->ChannelsMask, 4);

        RegionNvmGroup1->JoinChannelGroupsCurrentIndex = 0;
    }
    // Check other channels
    if (nextChanParams->Datarate >= DR_6) {
        if ((RegionNvmGroup1->ChannelsMaskRemaining[4] & CHANNELS_MASK_500KHZ_MASK)
            == 0) {
            RegionNvmGroup1->ChannelsMaskRemaining[4] = RegionNvmGroup2->ChannelsMask[4];
        }
    }
    ...
    status = RegionCommonIdentifyChannels(
        &identifyChannelsParam,
        aggregatedTimeOff,
        enabledChannels,
        &nbEnabledChannels,
        &nbRestrictedChannels,
        time);
    ...
    if (status == LORAMAC_STATUS_OK) {
        if (nextChanParams->Joined == true) {
            ...
            *channel = enabledChannels[randr(0, nbEnabledChannels - 1)];
            ...
            if (nextChanParams->Datarate == DR_2) {
                if (RegionBaseUSComputeNext125kHzJoinChannel(
                        (uint16_t *)RegionNvmGroup1->ChannelsMaskRemaining,
                        &RegionNvmGroup1->JoinChannelGroupsCurrentIndex,
                        channel)
            ...
            // 500kHz Channels (64 - 71) DR6
                ...
                uint8_t i = 0;
                while (((RegionNvmGroup1->ChannelsMaskRemaining[4]
                         & CHANNELS_MASK_500KHZ_MASK)
                        & (1 << i))
                       == 0) {
                    i++;
                }
        ...
        RegionCommonChanDisable(
            RegionNvmGroup1->ChannelsMaskRemaining, *channel, AU915_MAX_NB_CHANNELS);
    }
    return status;
}
